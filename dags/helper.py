...
def analyze_data(PostgresHook=None, DATABASE_URL: str = None):
    ...
    # Si aucun résultat, insérer des données de test
    if not result:
        test_data = [
            ('google', 0),
            ('facebook', 0),
        ...
        for company, pageviews in test_data:
            cursor.execute(
                "INSERT INTO pageviews (company, pageviews) VALUES (%s, %s)",
                (company, pageviews)
            )
        conn.commit()
        
        # Requête à nouveau pour obtenir les résultats
        cursor.execute("""
        SELECT company, MAX(pageviews)
        FROM pageviews
        GROUP BY company
